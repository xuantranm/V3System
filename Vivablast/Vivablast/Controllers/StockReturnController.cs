...
namespace Vivablast.Controllers
{
    ...
    {
        private static readonly ILog Log = LogManager.GetLogger(typeof(StockReturnController).FullName);

        private readonly ISystemService _systemService;
        private readonly IStockReturnService _service;

        public StockReturnController(ISystemService systemService, IStockReturnService service)
        {
        ...
        public ActionResult Index()
        {
            ...
            if (user.StockReturnR == 0) return RedirectToAction("Index", "Home");
        ...
        public void ExportToExcel(int page, int size, int store, int project, int stockType, string stockCode, string stockName, string srv, string fd, string td, string enable)
        {
            // Get the data to report on
            var masters = _service.ListCondition(page, size, store, project, stockType, stockCode, stockName, srv, fd,
                td, enable);
            var details = _service.ListConditionDetailExcel(page, size, store, project, stockType, stockCode, stockName, srv, fd,
                td, enable);
            ...
            foreach (var item in masters)
            {
                ...
                row.CreateCell(1).SetCellValue(item.SRV);
            ...
            foreach (var detail in details)
            {
                ...
                if (String.CompareOrdinal(detail.SRV, lastProductName) != 0)
                {
                    ...
                    productNameToShow = detail.SRV;
                    lastProductName = detail.SRV;
                ...
                row.CreateCell(4).SetCellValue(detail.ReturnDate.ToString("dd/MM/yyyy"));
        ...
        [HttpPost]
        public JsonResult Create(ReturnAddViewModel model)
        {
            if (model.V3 != true)
            {
        ...
        private JsonResult EditData(ReturnAddViewModel model)
        {
            try
            {
                ...
                _service.Insert(model.ReturnStockItemList, model.LoginId);
