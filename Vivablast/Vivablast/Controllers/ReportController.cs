...
namespace Vivablast.Controllers
{
    public class ReportController : Controller
    {
        ...
        {
            var model = _systemService.GetDynamicProjectGroupItemReport(page, size, projectId, stockTypeId, categoryId, stockCode, stockName, actionFag, supplierId, fd, td);
            return PartialView("Partials/_DynamicProjectGroupItemReport", model);
        }

        public void ExportLoadDynamicProject(int page, int size, int projectId, int stockTypeId, int categoryId, string stockCode, string stockName, string actionFag, int supplierId, string fd, string td)
        {
            // Get the data to report on
            var masters = _systemService.GetDynamicProjectReport(page, size, projectId, stockTypeId, categoryId, stockCode, stockName, actionFag, supplierId, fd, td);
            // Create a new workbook
            var workbook = new HSSFWorkbook();
            ...
            #region Master sheet
            var sheet = workbook.CreateSheet("DynamicProject");
            ...
            var numberRow = 1;
            // Add data rows
            foreach (var master in masters.DynamicProjectReports)
            {
                row = sheet.CreateRow(rowIndex);
                row.CreateCell(0).SetCellValue(numberRow);
                row.CreateCell(1).SetCellValue(master.Action);
                ...
                row.CreateCell(6).SetCellValue(master.StockName);
                row.CreateCell(7).SetCellValue(master.StockType);
            ...
            using (var exportData = new MemoryStream())
            {
                ...
                var saveAsFileName = string.Format("ProjectDynamic-{0}.xls", DateTime.Now.ToString("ddMMyyyyHHmmss")).Replace("/", "-");
        ...
        public void ExportLoadDynamicProjectGroupItems(int page, int size, int projectId, int stockTypeId, int categoryId, string stockCode, string stockName, string actionFag, int supplierId, string fd, string td)
        {
            // Get the data to report on
            var masters = _systemService.GetDynamicProjectGroupItemReport(page, size, projectId, stockTypeId, categoryId, stockCode, stockName, actionFag, supplierId, fd, td);
            // Create a new workbook
            var workbook = new HSSFWorkbook();
            ...
            #region Master sheet
            var sheet = workbook.CreateSheet("Dynamic Group Item");
            ...
            var numberRow = 1;
            // Add data rows
            foreach (var master in masters.DynamicProjectGroupItemReports)
            {
                row = sheet.CreateRow(rowIndex);
                row.CreateCell(0).SetCellValue(numberRow);
                row.CreateCell(1).SetCellValue(master.StockCode);
                row.CreateCell(2).SetCellValue(master.StockName);
                row.CreateCell(3).SetCellValue(master.StockType);
                row.CreateCell(4).SetCellValue(master.Category);
                row.CreateCell(5).SetCellValue(master.Unit);
                row.CreateCell(6).SetCellValue(master.QtyStockIn.ToString());
                row.CreateCell(7).SetCellValue(master.QtyStockOut.ToString());
                row.CreateCell(8).SetCellValue(master.QtyStockReturn.ToString());
                row.CreateCell(9).SetCellValue(master.QtyStockRemaining.ToString());
                row.CreateCell(10).SetCellValue(master.Weight);
                rowIndex++;
                numberRow++;
            }
            ...
            using (var exportData = new MemoryStream())
            {
                ...
                var saveAsFileName = string.Format("DynamicProjectGroupItem-{0}.xls", DateTime.Now.ToString("ddMMyyyyHHmmss")).Replace("/", "-");
